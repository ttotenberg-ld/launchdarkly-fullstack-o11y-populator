...
|---|---|---|---|---|
| `migrate-warehouse-api` | multivariate string (client + server eval) | `v1` / `v2` / `v3` | Server: `backend/services/inventory-service/app.py:151` (`get_warehouse_api_version`). Client: evaluated on mount in `frontend/src/App.jsx:18` to generate feature events. | Controls warehouse error injection. v1 = clean legacy baseline (`WAREHOUSE_V1_ERRORS` list is empty). v2 = ~93% composite error rate (TimeoutError, ParseError, RateLimit, StaleCache, Auth failures). v3 = clean (stabilized). Every injected error calls `record_exception` + `record_log` + increments `app.inventory.warehouse_error_total`. |
